...
class DaliUSB(object):
    ...
    def __init__(self,address=None,serial=None):
        ...
        dev=usb.core.find(custom_match=lambda x:
                          (x.idVendor,x.idProduct) in DaliUSB.id_list,
                          **kwargs)
        if not dev: raise DeviceNotFound
        self._dev=dev
        self._seqnum=1
    ...
    def send(self,command):
        ...
        ecommand=0
        a,b=command.command
        msg=struct.pack("BBBBBBBB"+(64-8)*'x',direction,seqnum,0,mtype,0,ecommand,a,b)
        print(repr(msg))

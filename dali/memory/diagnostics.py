...
Counts the control gear operating time in seconds if the control gear
is powered regardless of the status of lampOn bit.
"""
...
power on time is at least 600ms.
"""
...
ControlGearExternalSupplyVoltage < ControlGearExternalSupplyUndervoltage
threshold.
...
ControlGearExternalSupplyUndervoltage threshold shall be lower than
the lower end of the specified input voltage range of the control gear
and the value of the ControlGearExternalSupplyUndervoltage threshold
...
ControlGearExternalSupplyVoltage > ControlGearExternalSupplyOvervoltage
threshold.
...
ControlGearExternalSupplyOvervoltage threshold shall be higher than
the higher end of the specified input voltage range of the control
gear and the value of the ControlGearExternalSupplyOvervoltage
threshold is such that lifetime and/or performance of the control gear
could be affected if the ControlGearExternalSupplyVoltage is higher
than the threshold.
...
ControlGearOutputPowerLimitation threshold represents the output power
limit of the control gear.

NOTE ControlGearOutputPowerLimitation = 1 if the control gear limits
the output current due to its internal power limitation. This is the
case if the LED voltage multiplied with the control gear output
current is higher than the output power limit of the control gear.
"""
...
lifetime and/or performance of the control gear could be affected if
the ControlGearTemperature is higher than the threshold.
ControlGearThermalShutdown threshold shall be higher than
ControlGearThermalDerating threshold.

If ControlGearThermalShutdown = 1 the output current of the control
gear shall be reduced to zero.
"""
...
ControlGearOutputCurrentPercent includes the following reductions of
output current:
...
LightSourceOpenCircuit = 1 if the light source has a lamp failure with
open circuit according to QUERY LAMP FAILURE defined in
IEC62386-102:2014.
...
lifetime and/or performance of the light source could be affected if
the LightSourceTemperature is higher than the threshold.

If LightSourceThermalDerating = 1 the output current of the control
gear may be reduced.
"""
...
If LightSourceThermalShutdown = 1 the output current of the control
gear shall be reduced to zero.
"""
...
thermaly coupled to the light source.
